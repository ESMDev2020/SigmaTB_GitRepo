# -*- coding: utf-8 -*-
"""
OPTIMIZED DATABASE SYNCHRONIZATION SCRIPT
DOCUMENTATION: Synchronizes data between source database (AS400 or MySQL) and MSSQL 
               ...
               Data type preservation with optimized performance.
"""
...
myDictConfig = {
    ...
    # Operation settings
    'BATCH_SIZE': 10000,  # Back to original size
    'MAX_THREADS': 1,
    'MAX_RETRIES': 3,
    'RETRY_DELAY': 5,
    'LOG_FILE': "failed_inserts.log",
    'REPORT_FILE': "migration_report.csv",
    'DEBUG_MODE': 1  # New parameter: 1 = show messages, 0 = hide non-critical messages
}
...
# Thread-local operation counter (more efficient than global lock)
...
def get_operation_count():
    """
    Thread-local counter without locking overhead.
    Get current operation counter safely for this thread
    """
...
def fun_PrintStatus(varStrTableName, varStrStatus, varStrIcon):
    ...
    Now uses tqdm.write to avoid conflicts with progress bars.
    Respects DEBUG_MODE setting in myDictConfig.
    
    INPUT:
        varStrTableName - Name of table being processed
        varStrStatus - Status message to display
        varStrIcon - Icon type for visual indication (process, download, insert, etc.)
    """
    # If DEBUG_MODE is 0, don't print anything except errors and warnings
    if myDictConfig.get('DEBUG_MODE', 1) == 0 and varStrIcon not in ["failure", "warning"]:
        return
    ...
    # Get the icon or use question mark if not found
    varStrIcon = dictIcons.get(varStrIcon, "❓")
    
    # Format the message
    message = f"[{varStrTimestamp}] {varStrIcon} {varStrTableName.ljust(20)}: {varStrStatus}"
    
    # Use print_lock to ensure thread safety
    with print_lock:
        # Use tqdm.write to avoid conflicts with progress bars
        tqdm.write(message)

def fun_ParseCommandLineArgs():
    """
    Parse command line arguments to set configuration options.
    Allows setting DEBUG_MODE from command line.
    """
    ...
    parser.add_argument('--debug', type=int, choices=[0, 1], default=1,
                        help='Debug mode: 1=show all messages, 0=hide non-critical messages')
    
    args = parser.parse_args()
    
    # Update configuration with command line arguments
    myDictConfig['DEBUG_MODE'] = args.debug
    
    return args

# =============================================
# CORE DATABASE FUNCTIONS SECTION
# =============================================
...
def fun_GetOneColumnMetadata(varObjSourceCursor, varObjMSSQLCursor, varStrSchema, varStrTable):
    """
    OPTIMIZED: Cache metadata information to prevent repeated lookups.
    ...
    """
    # Add a static cache to this function if it doesn't exist yet
    if not hasattr(fun_GetOneColumnMetadata, "cache"):
        fun_GetOneColumnMetadata.cache = {}
    
    # Check if we already have metadata for this table
    cache_key = f"{varStrSchema}.{varStrTable}"
    if cache_key in fun_GetOneColumnMetadata.cache:
        return fun_GetOneColumnMetadata.cache[cache_key]
    
    try:
        # Different metadata queries based on source type
        if myDictConfig['SOURCE_TYPE'] == 'AS400':
            # Using a more efficient query that gets exactly what we need
            varObjSourceCursor.execute(f"""
                SELECT COLUMN_NAME, COALESCE(COLUMN_TEXT, COLUMN_NAME) AS COLUMN_DESC, 
                       DATA_TYPE, LENGTH, NUMERIC_PRECISION, NUMERIC_SCALE 
            ...
            varObjSourceCursor.execute(f"""
                SELECT COLUMN_NAME, COALESCE(COLUMN_COMMENT, COLUMN_NAME) AS COLUMN_DESC, 
        ...
        for col in varObjSourceCursor.fetchall():
            ...
            col_desc = (col[1] or col_name).strip().replace(" ", "_").replace("'", "_") \
                      .replace(']', ']]').replace('?', 'q').replace('/', '_')
            col_type = col[2].strip().upper() if col[2] else "VARCHAR"
            col_length = col[3] if col[3] is not None else 50
            col_precision = col[4] if col[4] is not None else 18
            col_scale = col[5] if col[5] is not None else 0
            
            varListMetadataCols.append((
                col_name, col_desc, col_type, col_length, col_precision, col_scale
            ))
        
        # Verify with actual columns for safety
        ...
        if len(varListMetadataCols) != len(varListActualCols):
            ...
            varListMetadataCols = [(col, col, "VARCHAR", 100, 18, 0) for col in varListActualCols]
        
        # Store in cache for future use
        fun_GetOneColumnMetadata.cache[cache_key] = varListMetadataCols
        return varListMetadataCols
        
    except Exception as varExcError:
        fun_PrintStatus(varStrTable, f"Metadata query failed: {str(varExcError)} - Using actual names", "warning")
        varListActualCols = fun_GetActualColumnNames(varObjSourceCursor, varStrSchema, varStrTable)
        result = [(col, col, "VARCHAR", 100, 18, 0) for col in varListActualCols]
        
        # Still cache the result to avoid repeated failures
        fun_GetOneColumnMetadata.cache[cache_key] = result
        return result
...
def fun_DetectFiscalYearColumn(varObjSourceCursor, varStrSchema, varStrTable):
    ...
    if not hasattr(fun_DetectFiscalYearColumn, "cache"):
        fun_DetectFiscalYearColumn.cache = {}
    
    cache_key = f"{varStrSchema}.{varStrTable}"
    if cache_key in fun_DetectFiscalYearColumn.cache:
        return fun_DetectFiscalYearColumn.cache[cache_key]
    ...
    try:
        ...
        result = varObjResult[0] if varObjResult else None
        
        # Cache the result
        fun_DetectFiscalYearColumn.cache[cache_key] = result
        return result
    except Exception as varExcError:
        fun_PrintStatus(varStrTable, f"Error detecting fiscal year column: {str(varExcError)}", "failure")
        # Cache the failure result too
        fun_DetectFiscalYearColumn.cache[cache_key] = None
        return None
...
def fun_MapAS400TypeToSQLType(varStrAS400Type, varIntLength, varIntPrecision, varIntScale):
    """
    OPTIMIZED: Simplified mapping with fewer edge cases.
    ...
    """
    # Add a static type mapping cache to avoid string manipulations
    if not hasattr(fun_MapAS400TypeToSQLType, "type_map"):
        fun_MapAS400TypeToSQLType.type_map = {
            'DECIMAL': 'decimal',
            'DEC': 'decimal', 
            'NUMERIC': 'decimal', 
            'NUM': 'decimal',
    ...
    # Get base type or default to varchar
    varStrType = varStrAS400Type.upper() if varStrAS400Type else "VARCHAR"
    base_type = fun_MapAS400TypeToSQLType.type_map.get(varStrType, "nvarchar")
    
    # Apply parameters based on base type
    if base_type == 'decimal':
        # Use a consistent large precision/scale for all numeric fields
        return f"DECIMAL(18,{varIntScale if varIntScale is not None else 0})"
    elif base_type in ['int', 'smallint', 'bigint']:
        return "INT"
    elif base_type == 'char':
        length = min(varIntLength if varIntLength is not None else 50, 8000)
        return f"CHAR({length})"
    elif base_type == 'varchar':
        length = varIntLength if varIntLength is not None else 50
        return "VARCHAR(MAX)" if length > 8000 else f"VARCHAR({length})"
...
def fun_CreateTable(varObjCursor, varStrSchema, varStrSqlTable, varListCols):
    ...
    try:
        ...
        fun_PrintStatus(varStrSqlTable, f"(#{get_operation_count()}) Creating table with proper data types", "process")
        varObjCursor.execute(sql)
        varObjCursor.connection.commit()
        return True
    except Exception as e:
        if "already an object named" in str(e):
            fun_PrintStatus(varStrSqlTable, f"(#{get_operation_count()}) Table already exists (possibly created by another process)", "warning")
            return True
        else:
            fun_PrintStatus(varStrSqlTable, f"(#{get_operation_count()}) Error creating table: {str(e)}", "failure")
            return False
...
def fun_BulkInsertYearRange(varObjSourceCursor, varObjMSSQLCursor, varStrSourceTable, varStrSqlTable, varStrYearCondition, varStrFiscalYearCol):
    """
    OPTIMIZED: Bulk insert data for a specific year range with minimal necessary conversions.
    ...
    # Get column metadata with type information ONCE for this table
    ...
    with tqdm(total=varIntTotalRecords, desc=f"Inserting {varStrSourceTable} {varStrYearCondition}") as varObjProgressBar:
        while True:
            ...
            # SIMPLIFIED CONVERSION: Only handle nulls and Decimals which often cause issues
            ...
            for row in varListBatch:
                ...
                for val in row:
                    if val is None:
                        ...
                        # Convert Decimal to float to avoid ODBC issues
                        normalized_row.append(float(val))
                    elif isinstance(val, datetime):
                        # Keep datetime as-is - SQL Server handles these well
            ...
            # Bulk insert with optimized error handling
            try:
                ...
                fun_PrintStatus(varStrSourceTable, f"(#{get_operation_count()}) Batch insert error: {str(e)}", "warning")
                
                # Progressive fallback: first try smaller batches before row-by-row
                batch_size = len(varListNormalizedBatch)
                if batch_size > 100:
                    fun_PrintStatus(varStrSourceTable, f"(#{get_operation_count()}) Trying with smaller batch size (100)", "info")
                    try:
                        # Process in smaller chunks of 100
                        successful_count = 0
                        for i in range(0, batch_size, 100):
                            chunk = varListNormalizedBatch[i:i+100]
                            try:
                                varObjMSSQLCursor.executemany(
                                    f"""
                                    INSERT INTO [{myDictConfig['SQL_SERVER']['SCHEMA']}].[{varStrSqlTable}]
                                ...
                                fun_PrintStatus(varStrSourceTable, f"(#{get_operation_count()}) Chunk insert error: {str(chunk_err)}", "warning")
                                # Only now fall back to row-by-row for this chunk
                                for row in chunk:
                                    try:
                                        ...
                                        pass  # Skip failed rows silently to avoid log spam
                        
                        varIntTotalInserted += successful_count
                        fun_PrintStatus(varStrSourceTable, 
                                      f"(#{get_operation_count()}) Smaller batch processing completed: {successful_count}/{batch_size} rows inserted", 
                                      "info")
                    except Exception as batch_err:
                        fun_PrintStatus(varStrSourceTable, f"(#{get_operation_count()}) Smaller batch error: {str(batch_err)}", "warning")
                        # Final fallback to row-by-row
                        successful_rows = 0
                        for row in varListNormalizedBatch:
                            try:
                                ...
                                pass  # Skip failed rows silently
                        
                        varIntTotalInserted += successful_rows
                        fun_PrintStatus(varStrSourceTable, 
                                      f"(#{get_operation_count()}) Row-by-row fallback completed: {successful_rows}/{batch_size} rows inserted", 
                                      "info")
                else:
                    # For small batches, go straight to row-by-row
                    successful_rows = 0
                    for row in varListNormalizedBatch:
                        try:
                            varObjMSSQLCursor.execute(
            ...
            # Commit less frequently to reduce overhead (every 30k rows)
            if varIntTotalInserted % (varIntBatchSize * 3) == 0:
                varObjMSSQLCursor.connection.commit()
                fun_PrintStatus(varStrSourceTable, 
                               f"(#{get_operation_count()}) Inserted {varIntTotalInserted} of {varIntTotalRecords} records", 
                               "update")
...
def fun_FullTableSync(varObjSourceCursor, varObjMSSQLCursor, varStrSourceTable, varStrSqlTable, dictResults):
    """
    OPTIMIZED: Perform full table sync when fiscal year column is not found.
    Handles both AS400 and MySQL as sources with simplified data conversions.
    ...
    """
    try:
        fun_PrintStatus(varStrSourceTable, f"(#{get_operation_count()}) Starting full table sync", "process")
        ...
        # Get column metadata with type information - ONCE for the table
        varListColumns = fun_GetOneColumnMetadata(
            varObjSourceCursor,
            varObjMSSQLCursor,
            varStrSourceSchema, 
        ...
        with tqdm(total=varIntTotalRecords, desc=f"Inserting {varStrSourceTable}") as varObjProgressBar:
            while True:
                ...
                # SIMPLIFIED CONVERSION: Only handle nulls and Decimals
                varListNormalizedBatch = []
                for row in varListBatch:
                    normalized_row = []
                    for val in row:
                ...
                # Bulk insert with progressive fallback strategy
                try:
                    varObjMSSQLCursor.fast_executemany = True
                    varObjMSSQLCursor.executemany(
                        f"""
        ...
        fun_PrintStatus(varStrSourceTable, f"(#{get_operation_count()}) Full table sync completed", "success")
        return dictResults
        
    except Exception as varExcError:
        fun_PrintStatus(varStrSourceTable, f"(#{get_operation_count()}) Full sync failed: {str(varExcError)}", "failure")
...
def fun_CompareAndSyncTables(varObjSourceCursor, varObjMSSQLCursor, varStrSourceTable, varStrSqlTable):
    """
    OPTIMIZED: Compare and sync tables year by year with detailed reporting.
    Reduced metadata calls and simplified type conversions for better performance.
    ...
    try:
        ...
        if not table_exists:
            fun_PrintStatus(varStrSourceTable, f"(#{get_operation_count()}) Destination table not found - creating it", "process")
            ...
            try:
                ...
                if table_created:
                    ...
                    fun_PrintStatus(varStrSourceTable, f"(#{get_operation_count()}) Destination table created successfully", "success")
                else:
                ...
                if "already an object named" in str(create_error):
                    fun_PrintStatus(varStrSourceTable, f"(#{get_operation_count()}) Table already exists (possibly created by another process)", "warning")
                else:
        ...
        fun_PrintStatus(varStrSourceTable, 
                       f"(#{get_operation_count()}) Initial counts - Source: {dictResults['initial_source_count']}, MSSQL: {dictResults['initial_mssql_count']}", 
                       "info")

        # If rowcount is equal, we skip
        if (dictResults['initial_source_count'] == dictResults['initial_mssql_count']):
            fun_PrintStatus(varStrSourceTable, 
                        f"(#{get_operation_count()}) Initial counts are equal - skipping...", 
                        "info")
        ...
        if not varStrFiscalYearCol:
            fun_PrintStatus(varStrSourceTable, f"(#{get_operation_count()}) No fiscal year column found - performing full table sync", "warning")
        ...
        try:
            ...
            fun_PrintStatus(varStrSqlTable, f"(#{get_operation_count()}) Error finding MSSQL fiscal column: {str(varExcError)}", "warning")
        ...
        for varStrLabel, varStrYearCondition in listYearRanges:
            ...
            fun_PrintStatus(
                varStrSourceTable,
                f"(#{get_operation_count()}) {varStrLabel} {varStrYearCondition} year range - Source: {varIntSourceCount}, MSSQL: {varIntMssqlCount}",
                "info"
            )

            # Only sync if counts differ and not empty
            if varIntSourceCount != varIntMssqlCount and varIntSourceCount > 0:
                fun_PrintStatus(varStrSourceTable, f"(#{get_operation_count()}) Counts differ - syncing {varStrLabel} year range", "process")
                ...
                # Insert fresh data from source - Using the optimized version
        ...
        fun_PrintStatus(varStrSourceTable, f"(#{get_operation_count()}) Getting final row counts", "process")
        ...
        fun_PrintStatus(varStrSourceTable, 
                       f"(#{get_operation_count()}) Final counts - Source: {dictResults['final_source_count']}, MSSQL: {dictResults['final_mssql_count']}", 
                       "info")
        ...
        fun_PrintStatus(varStrSourceTable, f"(#{get_operation_count()}) Sync failed: {str(varExcError)}", "failure")
        varObjMSSQLCursor.connection.rollback()
        dictResults['error'] = str(varExcError)
...
def fun_ProcessTable(varTupleTableInfo, varObjLogLock, varObjReportLock):
    ...
    try:
        ...
        with varObjLogLock:
            fun_PrintStatus(varStrSourceTable, f"(#{get_operation_count()}) Error: {str(varExcError)}", "failure")
            traceback.print_exc()
...
if __name__ == "__main__":
    try:
        # Parse command line arguments for DEBUG_MODE
        fun_ParseCommandLineArgs()
        
        # Initialize
        varStrStartTime = datetime.now().strftime(myConStrTimestampFormat)
        print(f"[{varStrStartTime}] Starting execution")
        fun_PrintStatus("SYSTEM", f"(#{get_operation_count()}) Starting {myDictConfig['SOURCE_TYPE']} to MSSQL migration process", "process")
        ...
        try:
            ...
            try:
                ...
                fun_PrintStatus("SYSTEM", f"(#{get_operation_count()}) Found {len(varListAllTables)} tables to process", "success")
                
            except Exception as varExcError:
                fun_PrintStatus("SYSTEM", f"(#{get_operation_count()}) Could not query equivalents table: {str(varExcError)}", "failure")
                varListAllTables = []
            ...
            while True:
                try:
                    ...
                    try:
                        ...
                        with varObjLogLock:
                            fun_PrintStatus("WORKER", f"(#{get_operation_count()}) Error processing table: {str(e)}", "failure")
                            traceback.print_exc()
                    finally:
                    ...
                    with varObjLogLock:
                        fun_PrintStatus("WORKER", f"(#{get_operation_count()}) Thread failed: {str(e)}", "failure")
                    return
        ...
        try:
            ...
            fun_PrintStatus("SYSTEM", f"(#{get_operation_count()}) Thread management error: {str(e)}", "failure")
        ...
        try:
            ...
            fun_PrintStatus("SYSTEM", f"(#{get_operation_count()}) Cleanup error: {str(e)}", "failure")
        ...
        fun_PrintStatus("SYSTEM", f"(#{get_operation_count()}) Migration completed successfully", "success")
        
    except Exception as varExcError:
        fun_PrintStatus("SYSTEM", f"(#{get_operation_count()}) Migration failed: {str(varExcError)}", "failure")
        traceback.print_exc()
