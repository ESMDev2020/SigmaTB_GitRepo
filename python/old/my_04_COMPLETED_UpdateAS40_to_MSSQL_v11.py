...
# Global operation counter with thread-safety
operation_counter = {'value': 0, 'lock': threading.Lock()}
...
def get_operation_count():
    """Get current operation counter safely"""
    with operation_counter['lock']:
        operation_counter['value'] += 1
        return operation_counter['value']
...
def fun_MapAS400TypeToSQLType(varStrAS400Type, varIntLength, varIntPrecision, varIntScale):
    ...
    if varStrType in ('DECIMAL', 'DEC', 'NUMERIC', 'NUM'):
        ...
        # Use larger precision for all numeric fields to prevent overflow
...
def fun_CompareRowCountPerYear(varObjSourceCursor, varObjMSSQLCursor, varStrSourceTable, varStrSqlTable):
    ...
    if not varStrSourceFiscalCol:
        fun_PrintStatus(varStrSourceTable, f"(#{get_operation_count()}) No fiscal year column found - using full table comparison", "warning")
    ...
    try:
        ...
        fun_PrintStatus(varStrSqlTable, f"(#{get_operation_count()}) Error finding MSSQL fiscal column: {str(varExcError)}", "failure")
        varStrMssqlFiscalCol = None
    
    if not varStrMssqlFiscalCol:
        fun_PrintStatus(varStrSqlTable, f"(#{get_operation_count()}) No matching MSSQL column found for {varStrSourceFiscalCol}", "warning")
        return {'FullTable': {'needs_sync': True}}
...
def fun_BatchInsertWithLimits(cursor, sql, parameters, max_rows_per_batch=100):
    """
    Insert data in batches with a limited number of rows per batch to avoid parameter limits
    ...
    for i in range(0, len(parameters), max_rows_per_batch):
        batch = parameters[i:i + max_rows_per_batch]
        
        # Use standard executemany for 2024 data to avoid freezes
...
def fun_BulkInsertYearRange(varObjSourceCursor, varObjMSSQLCursor, varStrSourceTable, varStrSqlTable, varStrYearCondition, varStrFiscalYearCol):
    ...
    # Adjust batch size based on the year condition
    if "GLPPYY = 24" in varStrYearCondition:
        # Smaller batch size for 2024 data to avoid freezes
        varIntBatchSize = 100
    else:
    ...
    # Debug output for 2024 data only
    if "GLPPYY = 24" in varStrYearCondition:
        fun_PrintStatus(varStrSourceTable, f"(#{get_operation_count()}) Using smaller batch size (100) for 2024 data", "info")
    ...
    with tqdm(total=varIntTotalRecords, desc=f"Inserting {varStrSourceTable} {varStrYearCondition}") as varObjProgressBar:
        while True:
            ...
            # Bulk insert with batch limiting
            try:
                # For 2024 data, use standard executemany and smaller batches
                if "GLPPYY = 24" in varStrYearCondition:
                    rows_inserted = fun_BatchInsertWithLimits(
                        varObjMSSQLCursor, 
                        f"""INSERT INTO [{myDictConfig['SQL_SERVER']['SCHEMA']}].[{varStrSqlTable}]
                           ({varStrColNames}) VALUES ({varStrPlaceholders})""",
                        varListNormalizedBatch,
                        100  # Even smaller sub-batches for 2024 data
                    )
                else:
                    # For other years, use standard fast insert
                    ...
                    rows_inserted = len(varListNormalizedBatch)
                
                varIntTotalInserted += rows_inserted
            except Exception as e:
                ...
                fun_PrintStatus(varStrSourceTable, f"(#{get_operation_count()}) Attempting row-by-row insert as fallback", "info")
                successful_rows = 0
                ...
                for row_idx, row in enumerate(varListNormalizedBatch):
                    try:
                        ...
                        fun_PrintStatus(varStrSourceTable, 
                                       f"(#{get_operation_count()}) Row insert error at batch position {row_idx}: {str(row_err)}", 
                                       "warning")
                
                fun_PrintStatus(varStrSourceTable, 
                               f"(#{get_operation_count()}) Row-by-row fallback completed: {successful_rows}/{len(varListNormalizedBatch)} rows inserted", 
                               "info")
            ...
            if varIntTotalInserted % (varIntBatchSize * 10) == 0:
                ...
                fun_PrintStatus(varStrSourceTable, 
                               f"(#{get_operation_count()}) Inserted {varIntTotalInserted} of {varIntTotalRecords} records for {varStrYearCondition}", 
                               "update")
...
def fun_FullTableSync(varObjSourceCursor, varObjMSSQLCursor, varStrSourceTable, varStrSqlTable, dictResults):
    ...
    try:
        ...
        varStrColNames = ", ".join([f"[{varTupleCol[1]}_____{varTupleCol[0]}]" for varTupleCol in varListColumns])
        varStrPlaceholders = ", ".join(["?"] * len(varListColumns))
        varIntBatchSize = 500  # Smaller batch size for full table sync
        varIntTotalInserted = 0
        ...
        with tqdm(total=varIntTotalRecords, desc=f"Inserting {varStrSourceTable}") as varObjProgressBar:
            while True:
                ...
                # Bulk insert with batch limiting
                try:
                    # Use smaller sub-batches for full table sync
                    rows_inserted = fun_BatchInsertWithLimits(
                        varObjMSSQLCursor, 
                        f"""INSERT INTO [{myDictConfig['SQL_SERVER']['SCHEMA']}].[{varStrSqlTable}]
                           ({varStrColNames}) VALUES ({varStrPlaceholders})""",
                        varListNormalizedBatch,
                        100  # Even smaller sub-batches for full table
                    )
    ...
    try:
        ...
        for varStrLabel, varStrYearCondition in listYearRanges:
            ...
            if varIntSourceCount != varIntMssqlCount:
                fun_PrintStatus(varStrSourceTable, f"(#{get_operation_count()}) Counts differ - syncing {varStrLabel} year range", "failure")
